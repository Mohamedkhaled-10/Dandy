...
var $$Product = createComponent(($$result, $$props, $$slots) => {
    ...
    db.ref("products/" + id).once("value").then(snapshot => {
      if (snapshot.exists() && !snapshot.val().isHidden) {
        ...
        try {
          ...
          const productSchema = {
            ...
            "offers": {
              "@type": "Offer",
              "url": window.location.origin + \`/product?id=\${encodeURIComponent(id)}\`,
              "priceCurrency": "EGP",
        ...
        const shareURL = window.location.origin + \`/product?id=\${encodeURIComponent(id)}\`;
...
if (hasVars && (!Array.isArray(selectedVariants) || selectedVariants.length === 0)) {
  ...
  return;
        ...
        if(products.length > 0) {
          ...
          products.forEach(prod => {
              ...
              let actionBtn = hasVars
                ...
                : \`<button class="add-cart-btn" onclick="event.stopPropagation(); addToCartCard('\${prod.id}', '\${(prod.name||'').replace(/'/g, "\\\\'")}', \${priceRaw}, '\${prod.image||''}', \${prod.discount||0}, \${prod.onSale||false}, false)">
    ...
    window.addToCartCard = function(id, name, price, image, discount, onSale, hasVariants) {
      if (hasVariants) {
