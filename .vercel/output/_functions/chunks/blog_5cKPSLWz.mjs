    ...
    async function loadArticles() {
      ...
      try {
        ...
        snapshot.forEach(doc => {
          ...
          // Display only articles that are not drafts (status !== 'draft') or legacy articles without a status field
          ...
          const dateText = formatArticleDate(data);
          ...
          const postLink = data.slug
            ? \`/blog/\${encodeURIComponent(data.slug)}\`
            : \`/post?id=\${doc.id}\`;
          ...
          card.innerHTML = \`
            ...
            <div class="blog-card-body">
              ...
              <h3>\${title}</h3>
              <p>\${excerpt}</p>
              <a href="\${postLink}" class="read-more-btn">
