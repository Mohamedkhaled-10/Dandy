    ...
    db.ref("products").on("value", snapshot => {
      ...
      allProductsData.forEach(data => {
        ...
        if (!searchQuery || name.includes(searchQuery) || desc.includes(searchQuery) || category.includes(searchQuery)) {
          ...
          let actionBtn = \`<button class="add-cart-btn" onclick="event.stopPropagation(); quickAdd('\${data.id}', \\\`\${(data.name||'').replace(/\`/g, '')}\\\`, '\${data.price}', '\${data.image}', '\${data.discount||''}', \${data.onSale||false}, false, '\${data.slug || ''}')">
          ...
          const isFavorited = window.userWishlistSet && window.userWishlistSet.has(data.id);
          ...
          productEl.innerHTML = \`
            <div class="product-image-wrap">
              \${badgeStr}
              <button type="button" class="btn-card-wishlist \${heartActive}" data-product-id="\${data.id}" onclick="event.stopPropagation(); window.toggleCardWishlist('\${data.id}', this);" title="إضافة للمفضلة">
