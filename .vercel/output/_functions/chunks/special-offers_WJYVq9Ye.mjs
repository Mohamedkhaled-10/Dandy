    ...
    db.ref("products").on("value", snapshot => {
      ...
      snapshot.forEach(child => {
        ...
        if (data.onSale === true && !data.isHidden) {
          ...
          productEl.onclick = function() { window.location.href = '/product?id=' + child.key; };
          ...
          let actionBtn = \`<button class="add-cart-btn" onclick="event.stopPropagation(); quickAdd('\${child.key}', \\\`\${(data.name || '').replace(/\`/g, '')}\\\`, '\${data.price}', '\${data.image}', '\${data.discount || ''}', \${data.onSale || false}, false)">
          ...
          if (hasVars) {
            actionBtn = \`<button class="add-cart-btn" style="background:var(--color-primary); color:#fff;" onclick="event.stopPropagation(); window.location.href='/product?id=\${child.key}'">
