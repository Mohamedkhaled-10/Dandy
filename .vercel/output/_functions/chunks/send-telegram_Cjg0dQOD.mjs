...
//#region src/pages/api/send-telegram.js
var send_telegram_exports = /* @__PURE__ */ __exportAll({
	ALL: () => ALL,
	POST: () => POST
});
async function POST({ request }) {
	try {
		const { order, orderId, lowStockAlert, productName, remainingQty } = await request.json().catch(() => ({})) || {};
		...
		let message = "";
		if (lowStockAlert) message = `⚠️ *تنبيه مخزون منخفض في متجر داندي!*
────────────────
📦 *المنتج:* ${productName || "-"}
📉 *الكمية المتبقية:* ${remainingQty ?? 0}
⏱ *الوقت:* ${(/* @__PURE__ */ new Date()).toLocaleString("ar-EG")}

🔗 *إدارة المنتجات:*
https://dandy-ebon.vercel.app/dashboard-product`;
		else {
			const dateText = order?.timestamp ? new Date(order.timestamp).toLocaleString("ar-EG") : (/* @__PURE__ */ new Date()).toLocaleString("ar-EG");
...
//#endregion
//#region \0virtual:astro:page:src/pages/api/send-telegram@_@js
var page = () => send_telegram_exports;
//#endregion
export { page };
