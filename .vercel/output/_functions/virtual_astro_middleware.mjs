import { t as sequence } from "./chunks/sequence_BGeKp411.mjs";
//#region src/middleware.js
async function onRequest$1({ url, redirect }, next) {
	...
	if (legacyMap[pathname]) return redirect(legacyMap[pathname] + url.search, 301);
	if (pathname.endsWith(".html") && !pathname.startsWith("/assets/")) {
		const cleanPath = pathname.replace(/\.html$/, "");
		return redirect((cleanPath === "/index" || cleanPath === "" ? "/" : cleanPath) + url.search, 301);
	}
	if (pathname.startsWith("/pages/")) {
		const segments = pathname.split("/").filter(Boolean);
		const lastSegment = (segments[segments.length - 1] || "").replace(/\.html$/, "");
		return redirect((lastSegment === "index" || lastSegment === "" ? "/" : "/" + lastSegment) + url.search, 301);
	}
	return next();
}
//#endregion
